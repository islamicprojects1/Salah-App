...
class FamilyController extends GetxController {
  final FamilyService _familyService = sl<FamilyService>();
  ...
  /// User ID we're currently sending encouragement to (for loading state).
  final Rx<String?> sendingEncouragementToUserId = Rx<String?>(null);
  ...
  final memberDailyLogs =
      <String, Map<String, String>>{}.obs; // userId -> { prayerName: quality }
  ...
  Future<void> addChild(String name, DateTime birthDate, String gender) async {
    ...
    if (nameErr != null) {
      AppFeedback.showSnackbar('alert'.tr, nameErr);
      return;
    }
    final birthErr = InputValidators.validateBirthDate(birthDate);
    if (birthErr != null) {
      AppFeedback.showSnackbar('alert'.tr, birthErr);
      return;
    }
  ...
  Future<void> pokeMember(
    String userId,
    String name, {
    String? customMessage,
  }) async {
    sendingEncouragementToUserId.value = userId;
    try {
      final message =
          customMessage ??
      ...
      sendingEncouragementToUserId.value = null;
    }
  }
