...
import 'package:salah/features/family/data/models/family_activity_model.dart';
...
class FamilyService extends GetxService {
  ...
  Future<bool> createFamily(String name) async {
    try {
      ...
      // Subscribe to FCM topic for family push notifications
      await _subscribeToFcmFamily(ref.id);
      ...
      AppLogger.error('FamilyService.createFamily failed', e);
  ...
  Future<bool> joinFamily(String code) async {
    try {
      ...
      // Subscribe to FCM topic for family push notifications
      await _subscribeToFcmFamily(doc.id);
      ...
      AppLogger.error('FamilyService.joinFamily failed', e);
  ...
  Future<bool> addChildWithoutPhone({
    ...
    try {
      ...
      AppLogger.error('FamilyService.addMember failed', e);
  ...
  Future<bool> logPrayerForMember({
    ...
    try {
      ...
      AppLogger.error('FamilyService.logPrayerForMember failed', e);
  ...
  Future<void> addPulseEvent({
    ...
    try {
      final notificationBody = type == 'prayer_logged'
          ...
          : type == 'encouragement'
              ...
              : type == 'family_celebration'
                  ? 'family_celebration_body'.trParams({
                      'family':
                          currentFamily.value?.name ?? 'family_pulse'.tr,
                    })
                  : 'family_pulse'.tr;
      ...
      await _firestore
          ...
          .add({
            ...
            'notificationTitle': 'family_pulse'.tr,
            'notificationBody': notificationBody,
          });

      // Send FCM push to family topic (Legacy Server Key in fcm_service.dart)
      try {
        sl<FcmService>().sendNotificationToFamily(
          familyId: familyId,
  ...
  Future<bool> sendEncouragement(String memberId, String message) async {
    try {
      ...
      AppLogger.warning('FamilyService.encourageMember failed', e);
      return false;
    }
  }
  ...
  /// Subscribe to FCM topic for a family (enables push notifications from that family).
  Future<void> _subscribeToFcmFamily(String familyId) async {
    try {
      await sl<FcmService>().subscribeToFamily(familyId);
    } catch (e) {
      AppLogger.warning('FamilyService: FCM subscribe failed', e);
    }
  }
    ...
    try {
      ...
      final userSub = userStream.listen((userDoc) async {
        ...
        final familiesSub = familiesStream.listen((querySnap) async {
          ...
          // Subscribe to FCM topics for all families (enables push notifications)
          for (final f in loadedFamilies) {
            await _subscribeToFcmFamily(f.id);
          }
      ...
      AppLogger.error('FamilyService._subscribeToFamilies failed', e);
    }
  }
  ...
  Future<void> createActivity({
    ...
    try {
      ...
      AppLogger.warning('FamilyService: Failed to create activity', e);
    }
  }
